...
%   $Revision: 1.1.8.11.2.1 $  $Date: 2011/07/18 00:37:47 $
...
if strcmp(get(figHandle, 'WindowStyle'), 'docked')
    warning(message('images:truesize:dockedFigure'));
    return
end
...
if (scale < 100)
    warning(message('images:truesize:imageTooBigForScreen', floor( scale )));
end
...
if (scale < 1)
    warning(message('images:truesize:imageTooBigForScreen', round( 100*scale )));
elseif (scale > 1)
    warning(message('images:truesize:imageTooSmallToScale', round( 100*scale )));
end
...
if nargin >= 2
    ...
    if (~isequal(size(imSize), [1 2]))
        error(message('images:truesize:reqsizeMustBe1by2'));
    end
...
if (isempty(axHandle))
    error(message('images:truesize:currentFigureMissingAxes'));
end
...
if isempty(h)
    error(message('images:truesize:noImagesOrSurfaces'));
end
